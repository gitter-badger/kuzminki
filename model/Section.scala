...
case class InsertMultipleValuesSec(valuesList: Seq[Seq[Any]]) extends Section {
  def expression = "VALUES %s"
  def render = {
    expression.format(
...
case class InsertWhereNotExistsSec(values: Seq[Any], table: ModelTable, where: WhereAllSec) extends Section {
